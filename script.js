...
function setLang(lang){
 ...
 document.documentElement.lang=lang;document.documentElement.dir=(lang==='en'||lang==='tr')?'ltr':'rtl'; document.title=({ku:'KURDTOKEN | کوردستان',fa:'KURDTOKEN | کردستان',en:'KURDTOKEN | Kurdistan',tr:'KURDTOKEN | Kürdistan',ar:'KURDTOKEN | كردستان'})[lang]||'KURDTOKEN';
 localStorage.setItem('kurdtoken-lang',lang);
 const open=document.querySelector('.region-card.active');
 if(open) showPart(open.dataset.part, false);
}
select.addEventListener('change',e=>setLang(e.target.value));select.value=currentLang;setLang(currentLang);
...
function renderCities(){
 ...
 function showCity(c,clicked){
  ...
  const p3=document.createElement('p');p3.innerHTML='<b>'+labels[currentLang].attractions+':</b> '+(c.attraction||'—');body.appendChild(p3);
